...
object AppLog {
    private const val TAG = "BavShieldVPN"
    ...
    fun readTail(maxLines: Int = 300): String {
        val localized = App.instanceOrNull()?.let { LocaleHelper.strings(it) }
        val file = logFile ?: return localized?.getString(work.bavshield.vpn.R.string.logs_not_initialized)
            ?: "Log file is not initialized"
        if (!file.exists() || file.length() == 0L) {
            return localized?.getString(work.bavshield.vpn.R.string.logs_empty) ?: "Log is empty"
        }
        return runCatching {
            file.readLines().takeLast(maxLines).joinToString("\n")
        }.getOrElse {
            localized?.getString(work.bavshield.vpn.R.string.logs_read_failed, it.message.orEmpty())
                ?: "Could not read the log: ${it.message}"
        }
    }
