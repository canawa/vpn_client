...
data class SubscriptionInfo(
    ...
    fun expireLabel(resources: Resources, nowMs: Long = System.currentTimeMillis()): String? {
        if (expire <= 0) return resources.getString(work.bavshield.vpn.R.string.subscription_unlimited)
        val nowSec = TimeUnit.MILLISECONDS.toSeconds(nowMs)
        val remainingSec = expire - nowSec
        if (remainingSec <= 0) return resources.getString(work.bavshield.vpn.R.string.subscription_expired)

        return SubscriptionExpireFormatter.formatRemaining(resources, remainingSec)
    }
...
fun formatTrafficSpeed(bytesPerSec: Long): String =
    "${formatTrafficBytes(bytesPerSec.coerceAtLeast(0))}/s"
