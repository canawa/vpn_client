...
class BoxService(
    ...
    internal fun onStartCommand(): Int {
        ...
        GlobalScope.launch(Dispatchers.IO) {
            try {
                ...
                stopServiceWithError(t.message ?: service.getString(work.bavshield.vpn.R.string.error_vpn_start_failed))
            }
        }
        return android.app.Service.START_NOT_STICKY
    }
    ...
    private suspend fun startService() {
        withContext(Dispatchers.Main) {
            notification.show(service.getString(work.bavshield.vpn.R.string.vpn_starting))
        }

        val configFile = App.configFile
        if (!configFile.exists()) {
            stopServiceWithError(service.getString(work.bavshield.vpn.R.string.error_config_missing))
            return
        }
        ...
        if (content.isBlank()) {
            stopServiceWithError(service.getString(work.bavshield.vpn.R.string.error_config_empty))
            return
        }
        ...
        try {
            ...
            stopServiceWithError(t.message ?: service.getString(work.bavshield.vpn.R.string.error_xray_start_failed))
            return
        }
        ...
        withContext(Dispatchers.Main) {
            notification.show(service.getString(work.bavshield.vpn.R.string.vpn_connected))
        }
    }
