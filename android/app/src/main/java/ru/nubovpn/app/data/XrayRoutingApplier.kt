...
object XrayRoutingApplier {
    ...
    /** Без catch-all правила трафик из TUN не попадает в proxy. */
    fun ensureDefaultProxyRule(config: JSONObject) {
        val routing = config.optJSONObject("routing") ?: return
        val rules = routing.optJSONArray("rules") ?: JSONArray().also { routing.put("rules", it) }
        for (i in 0 until rules.length()) {
            val rule = rules.optJSONObject(i) ?: continue
            if (isCatchAllRule(rule)) return
        }
        putFinalRule(routing, "proxy")
        AppLog.i("XrayRoutingApplier added default catch-all -> proxy")
    }
    ...
    private fun putFinalRule(routing: JSONObject, outboundTag: String) {
        ...
        for (i in 0 until rules.length()) {
            val rule = rules.optJSONObject(i) ?: continue
            if (!isCatchAllRule(rule)) filtered.put(rule)
        }
    ...
    private fun isCatchAllRule(rule: JSONObject): Boolean =
